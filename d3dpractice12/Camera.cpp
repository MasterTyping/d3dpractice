...
void CCamera::RegenerateViewMatrix()
{
	...
	//카메라를 갱신할때마다 절두체도 같이 갱신해준다(월드 좌표계로)
...
CThirdPersonCamera::CThirdPersonCamera(CCamera * pCamera):CCamera(pCamera)
{
	...
	{ 
		...
		이것은 스페이스-쉽 카메라의 로컬 y-축 벡터가 어떤 방향이든지 3인칭 카메라(대부분 사람인 경우)의 y축 벡터가 
		월드좌표의 y-축이 되도록 즉, 똑바로 서있는 형태로 설정한다는 의미이다.
		그리고 로컬 x-축 벡터와 로컬 z-축 벡터의 y-좌표가 0.0f가 되도록 한다. 
...
void CThirdPersonCamera::Update(XMFLOAT3 & xmf3LookAt, float fTimeElapsed)
{
	...
	//카메라 오프셋 벡터를 회전 행렬로 변환(회전)한다
		...
		약간의 시차를 두고 회전하는 효과를 구현하기 위한 것이다. m_fTimeLag가 
		1보다 크면 fTimeLagScale이 작아지고 실제 회전(이동)이 적 게 일어날 것이다.
...
{
	XMFLOAT3 pUpVector = m_pPlayer->GetUpVector();
	//현재 카메라의 위치에서 플레이어를 바라보기 위한 카메라 변환 행렬을 생성한다. 
	XMFLOAT4X4 mtxLookAt = Matrix4x4::LookAtLH(m_xmf3Position, vLookAt,pUpVector );
