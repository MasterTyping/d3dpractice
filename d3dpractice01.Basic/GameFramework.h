...
public:
	...
	void ProcessInput();
	void AnimateObjects();//프레임워크의 핵심(사용자 입력, 애니메이션, 렌더링)을 구성하는 함수이다.
	void FrameAdvance();
	...
	void mmCreateRenderTargetView();
	void CreateDepthStencilView();
	void CreateRenderTargetView();
private:
	HWND m_hWnd;
	int m_nWndClientWidth; int m_nWndClientHeight;
	
	//게임 프레임워크에서 다룰 타이머
	GameTimer	m_GameTimer;
	
	//다음프레임 레이트를 윈도우 캡션에 출력하기위한 문자열
	...
	ID3D12PipelineLibrary *m_pd3dPipelineState;
	ID3D12Resource *m_ppd3dRenderTargetBuffers[m_nSwapChainBuffers]; 
	ID3D12DescriptorHeap *m_pd3dRtvDescriptorHeap;
	UINT m_nRtvDescriptorIncrementSize; //렌더 타겟 버퍼, 서술자 힙 인터페이스 포인터, 렌더 타겟 서술자 원소의 크기이다.
	ID3D12Resource *m_pd3dDepthStencilBuffer; ID3D12DescriptorHeap *m_pd3dDsvDescriptorHeap;
	UINT m_nDsvDescriptorIncrementSize; //깊이-스텐실 버퍼, 서술자 힙 인터페이스 포인터, 깊이-스텐실 서술자 원소의 크기이다.
	ID3D12CommandQueue *m_pd3dCommandQueue; ID3D12CommandAllocator *m_pd3dCommandAllocator; 
	ID3D12GraphicsCommandList *m_pd3dCommandList; //명령 큐, 명령 할당자, 명령 리스트 인터페이스 포인터이다.
	ID3D12Fence *m_pd3dFence; UINT64 m_nFenceValue; HANDLE m_hFenceEvent; //펜스 인터페이스 포인터, 펜스의 값, 이벤트 핸들이다.
#if defined(_DEBUG)
