...
XMFLOAT3 CHeightMapImage::GetHeightMapNormal(int x, int z)
{
	...
	(x+1, z), (z, z+1)에 대한 픽셀 값을 사용하여 법선 벡터를 계산한다.*/
...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device * pd3dDevice,
	ID3D12GraphicsCommandList * pd3dCommandList, int xStart, int zStart, 
	int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void * pContext)
{
	...
	for (int i = 0, z = zStart; z < (zStart + nLength); z++)
	{
		for (int x = xStart; x < (xStart + nWidth); x++, i++)
		{
			...
			(z*m_xmf3Scale.z));
			XMFLOAT4 TempColor = OnGetColor(x, z, pContext);
			XMFLOAT4 xmf3Color = TempColor;
...
XMFLOAT4 CHeightMapGridMesh::OnGetColor(int x, int z, void * pContext)
{
	...
	XMFLOAT3 Temp1 = pHeightMapImage->GetHeightMapNormal(x, z);
	XMFLOAT3 Temp2 = pHeightMapImage->GetHeightMapNormal(x + 1, z);
	XMFLOAT3 Temp3 = pHeightMapImage->GetHeightMapNormal(x + 1, z + 1);
	XMFLOAT3 Temp4 = pHeightMapImage->GetHeightMapNormal(x, z + 1);
	float fScale = Vector3::DotProduct(Temp1,
		xmf3LightDirection);
	fScale += Vector3::DotProduct(Temp2,
		xmf3LightDirection);
	fScale += Vector3::DotProduct(Temp3,
		xmf3LightDirection);
	fScale += Vector3::DotProduct(Temp4,
		xmf3LightDirection);
