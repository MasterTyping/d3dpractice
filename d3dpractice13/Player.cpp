	...
	ID3D12RootSignature * pd3dGraphicsRootSignature, void * pContext, int nMeshes):CPlayer(nMeshes)
{
	...
	/*플레이어의 위치를 지형의 가운데(y-축 좌표는 지형의 높이보다 1500 높게)로 설정한다. 플레이어 위치 벡터의 y좌표가
	지형의 높이보다 크고 중력이 작용하도록 플레이어를 설정하였으므로 플레이어는 점차적으로 하강하게 된다.*/
	float fHeight = pTerrain->GetHeight(pTerrain->GetWidth()*0.5f,
			pTerrain->GetLength()*0.5f);
	XMFLOAT3 POSITION = XMFLOAT3(pTerrain->GetWidth()*0.5f, fHeight + 1500.0f,
		pTerrain->GetLength()*0.5f);
...
CCamera * CTerrainPlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	...
	case FIRST_PERSON_CAMERA:
		...
		XMFLOAT3 fgravity = XMFLOAT3(0.0f, -250.0f, 0.0f);
		SetGravity(fgravity);
	...
	case SPACESHIP_CAMERA:
		...
		XMFLOAT3 sgravity = XMFLOAT3(0.0f, 0.0f, 0.0f);
		SetGravity(sgravity);
		SetMaxVelocityXZ(300.0f);
		SetMaxVelocityY(400.0f);
		m_pCamera = OnChangeCamera(SPACESHIP_CAMERA, nCurrentCameraMode);
	...
	case THIRD_PERSON_CAMERA:
		SetFriction(250.0f);
		XMFLOAT3 tgravity = XMFLOAT3(0.0f, -250.0f, 0.0f);
