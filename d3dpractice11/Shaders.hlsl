...
float4 PSInstancing(VS_INSTANCING_OUTPUT input) : SV_TARGET{ return(input.color); }
...
//인스턴싱 데이터를 위한 구조체이다.
struct INSTANCEDGAMEOBJECTINFO { matrix m_mtxGameObject; float4 m_cColor; };
StructuredBuffer<INSTANCEDGAMEOBJECTINFO> gGameObjectInfos : register(t0);
struct VS_INSTANCING_INPUT { float3 position : POSITION; float4 color : COLOR; };
struct VS_INSTANCING_OUTPUT { float4 position : SV_POSITION; float4 color : COLOR; };
